...
const state = {
  ...
  submissionId: null, resultId: null, primaryResultId: null
};
...
function readSaved() {
  ...
  state.resultId = null;
  state.primaryResultId = null;
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null');
    if (!saved || !Array.isArray(saved.answers)) return;
    if (saved.accessId && saved.accessId !== AccessGate.getAccessId()) return;
    ...
    if ((saved.testCompleted === true || saved.result) && state.answers.every(answer => answer !== null)) {
      ...
      state.submissionId = /^[a-f0-9-]{36}$/i.test(saved.submissionId || '') ? saved.submissionId : crypto.randomUUID();
      state.resultId = typeof saved.resultId === 'string' ? saved.resultId : null;
      state.primaryResultId = typeof saved.primaryResultId === 'string' ? saved.primaryResultId : null;
      if (!matchesResult || !saved.accessId || !saved.submissionId) saveState();
    }
...
function saveState() {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({
      ...
      primaryResultId: state.primaryResultId,
      accessId: AccessGate.getAccessId(),
      result: state.result,
...
function showResultSync(message, error = false) {
  const box = document.getElementById('result-sync');
  if (!box) return;
  box.classList.toggle('error', error);
  box.querySelector('span').textContent = message;
  box.querySelector('button').hidden = !error;
}
function syncResult() {
  if (!state.testCompleted || !state.result) return Promise.resolve();
  ...
  const input = {
    submissionId,
    resultCode:state.resultCode,
    personalityType:state.result.archetype.id,
    matchedCharacters:state.result.figures.slice(0, 3).map(item => ({ name:item.name, match:item.match })),
    scores:Object.fromEntries(ResultCode.keys.map((name, index) => [name, state.result.scores[index]]))
  };
  showResultSync('正在将测试结果关联到本次购买资格……');
  syncPromise = AccessGate.saveResult(input).then(response => {
    if (state.submissionId !== submissionId) return;
    if (!response.success) throw new Error(response.reason || 'save_failed');
    state.resultId = response.resultId;
    state.primaryResultId = response.primaryResultId;
    saveState();
    showResultSync(`结果已保存到本次购买资格（${response.accessId}）。`);
  }).catch(() => {
    if (state.submissionId === submissionId) showResultSync('结果暂未同步到服务器。请检查网络并重试，当前答案仍保存在本机。', true);
  }).finally(() => {
    syncPromise = null;
    if (state.testCompleted && state.submissionId !== submissionId && !state.resultId) syncResult();
  });
  return syncPromise;
}
...
function renderResult() {
  ...
  document.getElementById('result-content').innerHTML = `
    ...
    <div id="result-sync" class="result-sync"><span>${state.resultId ? '结果已关联到本次购买资格。' : '正在将结果关联到本次购买资格……'}</span><button id="retry-result-sync" type="button" hidden>重新同步</button></div>
  ...
  document.getElementById('retry-result-sync').addEventListener('click', syncResult);
  syncResult();
  animateNumbers();
