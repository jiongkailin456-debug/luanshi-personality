...
function snapshot(result) {
  ...
  const reality = [
    { title:'朋友眼中的你', text:`朋友可能先注意到你的${high.name}：${high.gift}当他们真正需要回应时，你往往会用自己的方式出现。` },
    { title:'团队里的你', text:`在团队里，${archetype.strength}你更适合被看见的，是做决定时的思路，而不只是最后的结果。` },
    { title:'独处时的你', text:`没有外界催促时，你会重新整理自己的判断。${low.name}较少被你优先使用，也提醒你留意那些尚未尝试的做法。` }
  ];
  const risks = [
    { title:'可能的盲点', text:`${high.shadow}而当${low.name}暂时退到后面时，你可能更习惯用自己熟悉的办法处理一切。` },
    { title:'风险提醒', text:`作为${archetype.id}，${archetype.risk}一项长处用得过满，也会变成新的压力。` },
    { title:'给你的建议', text:`保留你的${high.name}，也为${low.name}留一点练习的空间。${archetype.cost}下一次选择时，不妨问问自己还有没有另一种节奏。` }
  ];
  return { scores, top, archetype, figures, figure, high, second, third, low, portrait, reality, risks };
}

function renderResult() {
  const data = snapshot(state.result);
  const { scores, top, archetype, figures, figure, portrait, reality, risks } = data;
  ...
  const layers = [
    { label:'第一底色', item:top[0], copy:DIMENSIONS[top[0].index].gift },
    { label:'隐藏天赋', item:top[1], copy:DIMENSIONS[top[1].index].gift },
    { label:'长期优势', item:top[2], copy:DIMENSIONS[top[2].index].gift }
  ].map((layer, i) => `<div class="layer-item"><div class="layer-marker">${i+1}</div><div><h3>${layer.label}<span>· ${DIMENSIONS[layer.item.index].name}</span></h3><p>${layer.copy}</p></div></div>`).join('');
  const blocks = items => items.map(item => `<div class="${items === reality ? 'reality-item' : 'risk-item'}"><h3>${item.title}</h3><p>${escapeHtml(item.text)}</p></div>`).join('');
  const others = figures.slice(1, 4).map(item => `<details class="figure-other"><summary><strong>${item.name}</strong><span class="other-right">${item.match}%</span></summary><p>${item.note}这只是题目选择与人物原型参数之间的相近程度。</p></details>`).join('');
  document.getElementById('result-content').innerHTML = `
    ...
    <section class="result-section"><h2><span class="section-num">03</span>你的三层底色</h2><div class="layer-list">${layers}</div></section>
