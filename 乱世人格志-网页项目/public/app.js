...
const state = {
  ...
  testCompleted: false, locked: false,
  paymentStatus: { status: 'unpaid', orderId: null, unlockedAt: null },
  premiumReport: null, premiumIdentity: null
};
let premiumPurchase = null;
try { premiumPurchase = JSON.parse(localStorage.getItem(PremiumPayment.storageKey) || 'null'); } catch (_) { /* 可继续使用免费测试。 */ }
...
function resetTransientVisualState() {
  ...
  const targets = [document.body, document.querySelector('.page-shell'), document.getElementById('question-card'), document.getElementById('premium-unlocked'), ...Object.values(screenEls)];
  targets.forEach(el => {
    if (!el) return;
...
function startNew() {
  ...
  state.result = null;
  state.testCompleted = false;
  state.paymentStatus = { status: 'unpaid', orderId: null, unlockedAt: null };
  state.premiumReport = null;
  state.premiumIdentity = null;
  state.locked = false;
  saveState();
  renderQuestion();
...
function renderResult() {
  ...
  state.premiumIdentity = PremiumPayment.identity(state.result);
  ...
  document.getElementById('result-content').innerHTML = `
    ...
    <section id="premium-area" class="premium-area">
      <div id="premium-locked" class="premium-locked">
        ...
        <p class="premium-length">完整版 · 约 2000～3000 字</p>
        ...
        <div class="premium-preview"><h3>你的关系模式</h3><p>你在人际关系里，并不总是依赖一时的情绪行动。你更在意一个人如何处理承诺、分歧与小事……</p><div class="preview-blur" aria-hidden="true">关系中的信任、冲突和边界，会在完整档案里逐层展开。</div><div class="preview-shade"><span aria-hidden="true">锁</span>完整关系解析约 300 字</div></div>
        <div class="premium-preview-titles"><span>恋爱互动模式</span><span>你的事业位置</span><span>压力下的真实反应</span><span>金钱决策方式</span><span>最大的隐藏盲点</span></div>
        <div class="premium-cta"><div><small>一次解锁当前测试结果</small><strong>¥${PremiumPayment.price.toFixed(1)}</strong></div><button class="button button-primary button-wide" id="premium-open" type="button">解锁完整人格档案 ¥${PremiumPayment.price.toFixed(1)}</button><button class="text-button premium-restore" id="premium-restore" type="button">恢复购买</button></div>
      </div>
      <div id="premium-unlocked" class="premium-unlocked" hidden></div>
    </section>
  ...
  document.getElementById('premium-open').addEventListener('click', openPaywall);
  document.getElementById('premium-restore').addEventListener('click', restorePremiumFromClick);
  if (premiumIsVerifiedForCurrentResult()) renderPremiumUnlocked(data, state.premiumReport, false);
  else tryRestorePremium();
  animateNumbers();
...
function renderShareCard(data) {
  ...
  panel.innerHTML = `<h2>你的乱世人格卡</h2><p>长按或截图保存，分享给朋友看看他们测出了谁。</p>
    <div class="share-card" role="img" aria-label="${figure.name}，${figure.match}%匹配，${archetype.id}人格分享卡">
      ...
      <div class="share-foot"><div><strong>20 道题测出你的乱世人格</strong><span>搜索：乱世人格志</span></div><div class="qr-placeholder" aria-label="二维码占位">扫码<br>占位</div></div>
...
function premiumIsVerifiedForCurrentResult() {
  return Boolean(state.premiumIdentity && state.premiumReport && state.paymentStatus.status === 'paid' &&
    premiumPurchase?.resultId === state.premiumIdentity.resultId && premiumPurchase?.resultHash === state.premiumIdentity.resultHash);
}
function applyPremiumReport(report, purchase, scroll = true) {
  if (!state.premiumIdentity || purchase.resultId !== state.premiumIdentity.resultId || purchase.resultHash !== state.premiumIdentity.resultHash) return;
  premiumPurchase = purchase;
  state.paymentStatus = { status:'paid', orderId:purchase.orderId, unlockedAt:purchase.unlockedAt };
  state.premiumReport = report;
  try { localStorage.setItem(PremiumPayment.storageKey, JSON.stringify(purchase)); } catch (_) { /* 本次仍可查看。 */ }
  renderPremiumUnlocked(snapshot(state.result), report, scroll);
}
async function tryRestorePremium() {
  const identity = state.premiumIdentity;
  if (!identity || !premiumPurchase || premiumPurchase.resultId !== identity.resultId || premiumPurchase.resultHash !== identity.resultHash || premiumIsVerifiedForCurrentResult()) return;
  state.paymentStatus = { status:'pending', orderId:premiumPurchase.orderId || null, unlockedAt:null };
  const restored = await PremiumPayment.restorePurchase(premiumPurchase, identity);
  if (state.premiumIdentity?.resultId !== identity.resultId) return;
  if (restored.status === 'paid') applyPremiumReport(restored.report, premiumPurchase, false);
  else state.paymentStatus = { status:'unpaid', orderId:null, unlockedAt:null };
}
function premiumSection(number, title, content) {
  return `<section class="premium-report-section"><h3><span>${String(number).padStart(2, '0')}</span>${escapeHtml(title)}</h3>${content}</section>`;
}
function premiumParagraph(value) { return `<p>${escapeHtml(value)}</p>`; }
function renderPremiumUnlocked(data, report, scroll) {
  const locked = document.getElementById('premium-locked');
  const unlocked = document.getElementById('premium-unlocked');
  if (!locked || !unlocked || !premiumIsVerifiedForCurrentResult()) return;
  const matches = report.historicalMatches.map((item, i) => `<div class="premium-match"><strong>${i + 1}. ${escapeHtml(item.name)} <span>${item.match}%</span></strong>${premiumParagraph(item.similarity)}${premiumParagraph(item.difference)}</div>`).join('');
  unlocked.innerHTML = `
    <div class="premium-unlocked-head"><span class="unlocked-seal" aria-hidden="true">开</span><div><small>已解锁 · 专属结果</small><h2>完整人格档案</h2></div></div>
    <p class="premium-unlocked-intro">${escapeHtml(data.figure.name)} · ${escapeHtml(data.archetype.id)} · ${escapeHtml(state.premiumIdentity.resultId)}</p>
    ${premiumSection(1, report.overview.title, premiumParagraph(report.overview.text))}
    ${premiumSection(2, '三位最相近历史人物', matches)}
    ${premiumSection(3, report.relationships.title, `<strong class="premium-keyword">${escapeHtml(report.relationships.keyword)}</strong>${premiumParagraph(report.relationships.text)}`)}
    ${premiumSection(4, report.loveStyle.title, `<strong class="premium-keyword">${escapeHtml(report.loveStyle.keyword)}</strong>${premiumParagraph(report.loveStyle.text)}`)}
    ${premiumSection(5, report.teamRole.title, `<strong class="premium-keyword">${escapeHtml(report.teamRole.position)}</strong>${premiumParagraph(report.teamRole.text)}`)}
    ${premiumSection(6, report.career.title, `<strong class="premium-keyword">${escapeHtml(report.career.keyword)}</strong>${premiumParagraph(report.career.text)}`)}
    ${premiumSection(7, report.moneyStyle.title, `<strong class="premium-keyword">${escapeHtml(report.moneyStyle.keyword)}</strong>${premiumParagraph(report.moneyStyle.text)}`)}
    ${premiumSection(8, report.pressureMode.title, `<div class="pressure-steps"><p><b>平时</b>${escapeHtml(report.pressureMode.normal)}</p><p><b>中度压力</b>${escapeHtml(report.pressureMode.medium)}</p><p><b>高压时</b>${escapeHtml(report.pressureMode.high)}</p></div>${premiumParagraph(report.pressureMode.text)}<p><b>恢复建议：</b>${escapeHtml(report.pressureMode.recovery)}</p>`)}
    ${premiumSection(9, report.blindSpots.title, `<div class="blind-chain">${escapeHtml(report.blindSpots.chain)}</div>${premiumParagraph(report.blindSpots.text)}`)}
    ${premiumSection(10, report.risks.title, premiumParagraph(report.risks.text))}
    ${premiumSection(11, report.growthAdvice.title, premiumParagraph(report.growthAdvice.text))}
    ${premiumSection(12, report.historicalRole.title, `<div class="historical-role">${escapeHtml(report.historicalRole.name)}</div>${premiumParagraph(report.historicalRole.text)}`)}
    ${premiumSection(13, report.finalSummary.title, `<blockquote>“${escapeHtml(report.finalSummary.quote)}”</blockquote>${premiumParagraph(report.finalSummary.text)}<small>${escapeHtml(report.finalSummary.credit)}</small>`)}
    <section class="premium-report-section"><h3><span>14</span>完整人格档案分享卡</h3><p>把你的乱世身份、关系与事业关键词，留在一张可以截图的卡片里。</p><button class="button button-secondary button-wide" id="premium-share-button" type="button">生成完整档案分享卡</button><div id="premium-share-panel" hidden></div></section>`;
  locked.hidden = true;
  unlocked.hidden = false;
  unlocked.classList.add('is-revealing');
  afterDelay(() => unlocked.classList.remove('is-revealing'), 500);
  document.getElementById('premium-share-button').addEventListener('click', () => renderPremiumShareCard(data, report));
  if (scroll) unlocked.scrollIntoView({ behavior:'smooth', block:'start' });
}
function renderPremiumShareCard(data, report) {
  if (!premiumIsVerifiedForCurrentResult()) return;
  const panel = document.getElementById('premium-share-panel');
  panel.hidden = false;
  panel.innerHTML = `<div class="share-card premium-share-card" role="img" aria-label="${escapeHtml(data.figure.name)}完整人格档案分享卡">
    <div class="share-head"><div><div class="share-brand">乱世人格志</div><div class="share-subtitle">完整人格档案 · 已解锁</div></div><div class="share-stamp">人<br>格</div></div>
    <div class="share-body"><div class="share-percent">${data.figure.match}<small>%</small></div><div class="share-person">${escapeHtml(data.figure.name)}</div><div class="share-type">${escapeHtml(data.archetype.id)} · ${escapeHtml(report.historicalRole.name)}</div><div class="share-tags"><span>关系：${escapeHtml(report.relationships.keyword)}</span><span>事业：${escapeHtml(report.career.keyword)}</span></div><div class="share-voice">“${escapeHtml(report.finalSummary.quote)}”</div><div class="share-credit">${escapeHtml(report.finalSummary.credit)}</div></div>
    <div class="share-foot"><div><strong>完整人格档案</strong><span>搜索：乱世人格志</span></div><div class="qr-placeholder">扫码<br>占位</div></div></div>`;
  panel.scrollIntoView({ behavior:'smooth', block:'start' });
}

let paywallPreviousFocus = null;
function paywallMessage(message) { document.getElementById('paywall-message').textContent = message; }
function openPaywall() {
  if (!state.result) return;
  paywallPreviousFocus = document.activeElement;
  const data = snapshot(state.result);
  document.getElementById('paywall-result').innerHTML = `<span>你的结果</span><strong>${data.figure.match}% · ${escapeHtml(data.figure.name)}</strong><em>${escapeHtml(data.archetype.id)}</em>`;
  document.getElementById('paywall-price').textContent = `¥${PremiumPayment.price.toFixed(1)}`;
  document.getElementById('paywall-confirm').textContent = `立即解锁 ¥${PremiumPayment.price.toFixed(1)}`;
  paywallMessage('');
  const slot = document.getElementById('paywall-dev-slot');
  slot.replaceChildren();
  if (PremiumPayment.devMode) {
    const devButton = document.createElement('button');
    devButton.type = 'button';
    devButton.className = 'dev-payment-button';
    devButton.textContent = '开发测试：模拟已付款';
    devButton.addEventListener('click', simulatePayment);
    slot.append(devButton);
  }
  document.getElementById('paywall').hidden = false;
  document.body.classList.add('modal-open');
  document.getElementById('paywall-confirm').focus();
}
function closePaywall() {
  document.getElementById('paywall').hidden = true;
  document.body.classList.remove('modal-open');
  if (paywallPreviousFocus?.isConnected) paywallPreviousFocus.focus();
}
async function requestRealPayment() {
  const response = await PremiumPayment.createOrder(state.premiumIdentity);
  if (response.status === 'payment_not_configured') paywallMessage('完整档案支付功能即将开放');
  else paywallMessage('当前暂时无法创建订单，请稍后再试。');
}
async function restorePremiumFromClick() {
  if (document.getElementById('paywall').hidden) openPaywall();
  if (!premiumPurchase || premiumPurchase.resultId !== state.premiumIdentity?.resultId) { paywallMessage('支付系统尚未开放'); return; }
  paywallMessage('正在核对购买记录…');
  const identity = state.premiumIdentity;
  const response = await PremiumPayment.restorePurchase(premiumPurchase, identity);
  if (state.premiumIdentity?.resultId !== identity.resultId) return;
  if (response.status === 'paid') { applyPremiumReport(response.report, premiumPurchase); closePaywall(); }
  else paywallMessage('支付系统尚未开放，当前没有可恢复的已验证购买。');
}
async function simulatePayment() {
  if (!PremiumPayment.devMode || !state.premiumIdentity) return;
  const identity = state.premiumIdentity;
  state.paymentStatus = { status:'pending', orderId:null, unlockedAt:null };
  paywallMessage('正在验证开发测试订单…');
  const simulated = await PremiumPayment.simulateDevPayment(identity);
  if (simulated.status !== 'paid' || !simulated.paymentToken) {
    state.paymentStatus = { status:'failed', orderId:null, unlockedAt:null };
    paywallMessage('开发测试服务未开启，请使用本地开发服务。');
    return;
  }
  const verified = await PremiumPayment.unlockPremium({ ...identity, paymentToken:simulated.paymentToken });
  if (state.premiumIdentity?.resultId !== identity.resultId) return;
  if (verified.status !== 'paid') {
    state.paymentStatus = { status:'failed', orderId:simulated.orderId, unlockedAt:null };
    paywallMessage('测试订单未通过服务端验证。');
    return;
  }
  const purchase = { version:1, resultId:identity.resultId, orderId:simulated.orderId, status:'paid', unlockedAt:simulated.unlockedAt, resultHash:identity.resultHash, paymentToken:simulated.paymentToken };
  applyPremiumReport(verified.report, purchase);
  closePaywall();
}

document.getElementById('paywall-x').addEventListener('click', closePaywall);
document.getElementById('paywall-backdrop').addEventListener('click', closePaywall);
document.getElementById('paywall-later').addEventListener('click', closePaywall);
document.getElementById('paywall-confirm').addEventListener('click', requestRealPayment);
document.getElementById('paywall-restore').addEventListener('click', restorePremiumFromClick);
document.addEventListener('keydown', event => { if (event.key === 'Escape' && !document.getElementById('paywall').hidden) closePaywall(); });
